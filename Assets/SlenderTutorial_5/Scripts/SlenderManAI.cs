...
public class SlenderManAI : MonoBehaviour
{
    ...
    public float killRange = 2f; // Distance within which Slenderman kills the player
    public GameObject winCanvas; // Reference to the quit screen UI canvas
    ...
    private void Update()
    {
        ...
        // Check if Slenderman is within kill range of the player
    ...
    {
        Debug.Log("Player has been caught by Slenderman!");
        Time.timeScale = 0; // Pause the game
        Cursor.lockState = CursorLockMode.None; // Unlock the cursor
        Cursor.visible = true; // Make the cursor visible
        if (winCanvas != null)
        {
            winCanvas.SetActive(true); // Show the quit screen UI
        }
    }
}
